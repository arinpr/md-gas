            ...
            <div className="flex">
              ...
              <div className="hidden sm:-my-px sm:ms-10 sm:flex items-center space-x-8">
                {nav.map((node) => {
                  if (node.type === "link") {
                    return (
                      <NavLink key={node.key} href={node.href} active={node.isActive?.()}>
                        {node.label}
                      </NavLink>
                    );
                  }
                  ...
                  return (
                    <div key={node.key} className="relative group h-full flex items-center">
                      <button
                        type="button"
                        className={cn(
        ...
        <div className={(showingNavigationDropdown ? "block" : "hidden") + " sm:hidden"}>
          <div className="space-y-1 pb-3 pt-2">
            {nav.map((node) => {
              if (node.type === "link") {
                return (
                  <ResponsiveNavLink key={node.key} href={node.href} active={node.isActive?.()}>
                    {node.label}
                  </ResponsiveNavLink>
                );
              }
